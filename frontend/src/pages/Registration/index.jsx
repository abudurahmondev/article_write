...
export const Registration = () => {
  ...
  const { register, handleSubmit, formState: { errors, isValid } } = useForm({
    defaultValues: {
      fullName: 'Abdurahmon Xamroqulov',
      email: 'abdurahmon@test.uz',
      password: '12345678'
    },
    mode: 'onChange'
  })
  ...
  const onSubmit = async (values) => {
    ...
    if(!data.payload){
      return alert('Royhatdan otib bolmadi')
    }
  ...
  return (
    <Paper classes={{ root: styles.root }}>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {...register('fullName', { required: 'Toliq ismni kiritng' })}
      ...
      <TextField error={Boolean(errors.email?.message)}
        ...
        type="email"
        {...register('email', { required: 'Pochtani kiriting' })}
        className={styles.field} 
        label="E-Mail" 
        fullWidth 
        />
